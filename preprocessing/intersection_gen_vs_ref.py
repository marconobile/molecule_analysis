'''
Computes intersection between 2 smile lists (loaded from file) - either .smiles or .txt
Generates a .txt file where for each line you have:
df1: smi1, df2 smi2, tanimoto (always 1 here), natoms: n
where smi1 == smi2

usual order: 
- file1 -> generated smiles or shortest file
- file2 -> ref smiles or longer file
...
try:
    from utils.data_utils import flatten_list_of_lists, create_log, append_line_to_log, argparser, read_smiles_from_file, get_filename, TimeCode, apply_f_parallelized_batched
except:
    import sys
    sys.path.append("..")
    from utils.data_utils import flatten_list_of_lists, create_log, append_line_to_log, argparser, read_smiles_from_file, get_filename, TimeCode, apply_f_parallelized_batched
